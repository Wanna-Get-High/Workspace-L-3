// $ANTLR 3.4 Init.g 2011-12-05 19:40:21

package init.analyseurs;


import org.antlr.runtime.*;
import java.util.Stack;
import java.util.List;
import java.util.ArrayList;

@SuppressWarnings({"all", "warnings", "unchecked"})
public class InitLexer extends Lexer {
    public static final int EOF=-1;
    public static final int T__9=9;
    public static final int T__10=10;
    public static final int T__11=11;
    public static final int T__12=12;
    public static final int T__13=13;
    public static final int T__14=14;
    public static final int BLANCS=4;
    public static final int ENTIER=5;
    public static final int IDENT=6;
    public static final int LETTRE=7;
    public static final int NEWLINE=8;

    public boolean erreur = false;
    @Override
    public void reportError(RecognitionException e) {
      super.reportError(e);
      this.erreur = true;
    }


    // delegates
    // delegators
    public Lexer[] getDelegates() {
        return new Lexer[] {};
    }

    public InitLexer() {} 
    public InitLexer(CharStream input) {
        this(input, new RecognizerSharedState());
    }
    public InitLexer(CharStream input, RecognizerSharedState state) {
        super(input,state);
    }
    public String getGrammarFileName() { return "Init.g"; }

    // $ANTLR start "T__9"
    public final void mT__9() throws RecognitionException {
        try {
            int _type = T__9;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:14:6: ( ',' )
            // Init.g:14:8: ','
            {
            match(','); 

            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "T__9"

    // $ANTLR start "T__10"
    public final void mT__10() throws RecognitionException {
        try {
            int _type = T__10;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:15:7: ( ':=' )
            // Init.g:15:9: ':='
            {
            match(":="); 



            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "T__10"

    // $ANTLR start "T__11"
    public final void mT__11() throws RecognitionException {
        try {
            int _type = T__11;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:16:7: ( ';' )
            // Init.g:16:9: ';'
            {
            match(';'); 

            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "T__11"

    // $ANTLR start "T__12"
    public final void mT__12() throws RecognitionException {
        try {
            int _type = T__12;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:17:7: ( 'int' )
            // Init.g:17:9: 'int'
            {
            match("int"); 



            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "T__12"

    // $ANTLR start "T__13"
    public final void mT__13() throws RecognitionException {
        try {
            int _type = T__13;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:18:7: ( 'program' )
            // Init.g:18:9: 'program'
            {
            match("program"); 



            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "T__13"

    // $ANTLR start "T__14"
    public final void mT__14() throws RecognitionException {
        try {
            int _type = T__14;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:19:7: ( 'read' )
            // Init.g:19:9: 'read'
            {
            match("read"); 



            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "T__14"

    // $ANTLR start "LETTRE"
    public final void mLETTRE() throws RecognitionException {
        try {
            // Init.g:56:16: ( ( 'a' .. 'z' | 'A' .. 'Z' ) )
            // Init.g:
            {
            if ( (input.LA(1) >= 'A' && input.LA(1) <= 'Z')||(input.LA(1) >= 'a' && input.LA(1) <= 'z') ) {
                input.consume();
            }
            else {
                MismatchedSetException mse = new MismatchedSetException(null,input);
                recover(mse);
                throw mse;
            }


            }


        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "LETTRE"

    // $ANTLR start "ENTIER"
    public final void mENTIER() throws RecognitionException {
        try {
            int _type = ENTIER;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:57:8: ( ( '0' .. '9' )+ )
            // Init.g:57:12: ( '0' .. '9' )+
            {
            // Init.g:57:12: ( '0' .. '9' )+
            int cnt1=0;
            loop1:
            do {
                int alt1=2;
                int LA1_0 = input.LA(1);

                if ( ((LA1_0 >= '0' && LA1_0 <= '9')) ) {
                    alt1=1;
                }


                switch (alt1) {
            	case 1 :
            	    // Init.g:
            	    {
            	    if ( (input.LA(1) >= '0' && input.LA(1) <= '9') ) {
            	        input.consume();
            	    }
            	    else {
            	        MismatchedSetException mse = new MismatchedSetException(null,input);
            	        recover(mse);
            	        throw mse;
            	    }


            	    }
            	    break;

            	default :
            	    if ( cnt1 >= 1 ) break loop1;
                        EarlyExitException eee =
                            new EarlyExitException(1, input);
                        throw eee;
                }
                cnt1++;
            } while (true);


            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "ENTIER"

    // $ANTLR start "NEWLINE"
    public final void mNEWLINE() throws RecognitionException {
        try {
            int _type = NEWLINE;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:58:8: ( ( '\\r' )? '\\n' )
            // Init.g:58:10: ( '\\r' )? '\\n'
            {
            // Init.g:58:10: ( '\\r' )?
            int alt2=2;
            int LA2_0 = input.LA(1);

            if ( (LA2_0=='\r') ) {
                alt2=1;
            }
            switch (alt2) {
                case 1 :
                    // Init.g:58:10: '\\r'
                    {
                    match('\r'); 

                    }
                    break;

            }


            match('\n'); 

             skip(); 

            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "NEWLINE"

    // $ANTLR start "BLANCS"
    public final void mBLANCS() throws RecognitionException {
        try {
            int _type = BLANCS;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:59:9: ( ( ' ' | '\\t' )+ )
            // Init.g:59:13: ( ' ' | '\\t' )+
            {
            // Init.g:59:13: ( ' ' | '\\t' )+
            int cnt3=0;
            loop3:
            do {
                int alt3=2;
                int LA3_0 = input.LA(1);

                if ( (LA3_0=='\t'||LA3_0==' ') ) {
                    alt3=1;
                }


                switch (alt3) {
            	case 1 :
            	    // Init.g:
            	    {
            	    if ( input.LA(1)=='\t'||input.LA(1)==' ' ) {
            	        input.consume();
            	    }
            	    else {
            	        MismatchedSetException mse = new MismatchedSetException(null,input);
            	        recover(mse);
            	        throw mse;
            	    }


            	    }
            	    break;

            	default :
            	    if ( cnt3 >= 1 ) break loop3;
                        EarlyExitException eee =
                            new EarlyExitException(3, input);
                        throw eee;
                }
                cnt3++;
            } while (true);


             skip(); 

            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "BLANCS"

    // $ANTLR start "IDENT"
    public final void mIDENT() throws RecognitionException {
        try {
            int _type = IDENT;
            int _channel = DEFAULT_TOKEN_CHANNEL;
            // Init.g:60:7: ( LETTRE ( LETTRE | ENTIER )* )
            // Init.g:60:9: LETTRE ( LETTRE | ENTIER )*
            {
            mLETTRE(); 


            // Init.g:60:16: ( LETTRE | ENTIER )*
            loop4:
            do {
                int alt4=3;
                int LA4_0 = input.LA(1);

                if ( ((LA4_0 >= 'A' && LA4_0 <= 'Z')||(LA4_0 >= 'a' && LA4_0 <= 'z')) ) {
                    alt4=1;
                }
                else if ( ((LA4_0 >= '0' && LA4_0 <= '9')) ) {
                    alt4=2;
                }


                switch (alt4) {
            	case 1 :
            	    // Init.g:60:17: LETTRE
            	    {
            	    mLETTRE(); 


            	    }
            	    break;
            	case 2 :
            	    // Init.g:60:24: ENTIER
            	    {
            	    mENTIER(); 


            	    }
            	    break;

            	default :
            	    break loop4;
                }
            } while (true);


            }

            state.type = _type;
            state.channel = _channel;
        }
        finally {
        	// do for sure before leaving
        }
    }
    // $ANTLR end "IDENT"

    public void mTokens() throws RecognitionException {
        // Init.g:1:8: ( T__9 | T__10 | T__11 | T__12 | T__13 | T__14 | ENTIER | NEWLINE | BLANCS | IDENT )
        int alt5=10;
        switch ( input.LA(1) ) {
        case ',':
            {
            alt5=1;
            }
            break;
        case ':':
            {
            alt5=2;
            }
            break;
        case ';':
            {
            alt5=3;
            }
            break;
        case 'i':
            {
            int LA5_4 = input.LA(2);

            if ( (LA5_4=='n') ) {
                int LA5_11 = input.LA(3);

                if ( (LA5_11=='t') ) {
                    int LA5_14 = input.LA(4);

                    if ( ((LA5_14 >= '0' && LA5_14 <= '9')||(LA5_14 >= 'A' && LA5_14 <= 'Z')||(LA5_14 >= 'a' && LA5_14 <= 'z')) ) {
                        alt5=10;
                    }
                    else {
                        alt5=4;
                    }
                }
                else {
                    alt5=10;
                }
            }
            else {
                alt5=10;
            }
            }
            break;
        case 'p':
            {
            int LA5_5 = input.LA(2);

            if ( (LA5_5=='r') ) {
                int LA5_12 = input.LA(3);

                if ( (LA5_12=='o') ) {
                    int LA5_15 = input.LA(4);

                    if ( (LA5_15=='g') ) {
                        int LA5_18 = input.LA(5);

                        if ( (LA5_18=='r') ) {
                            int LA5_20 = input.LA(6);

                            if ( (LA5_20=='a') ) {
                                int LA5_22 = input.LA(7);

                                if ( (LA5_22=='m') ) {
                                    int LA5_23 = input.LA(8);

                                    if ( ((LA5_23 >= '0' && LA5_23 <= '9')||(LA5_23 >= 'A' && LA5_23 <= 'Z')||(LA5_23 >= 'a' && LA5_23 <= 'z')) ) {
                                        alt5=10;
                                    }
                                    else {
                                        alt5=5;
                                    }
                                }
                                else {
                                    alt5=10;
                                }
                            }
                            else {
                                alt5=10;
                            }
                        }
                        else {
                            alt5=10;
                        }
                    }
                    else {
                        alt5=10;
                    }
                }
                else {
                    alt5=10;
                }
            }
            else {
                alt5=10;
            }
            }
            break;
        case 'r':
            {
            int LA5_6 = input.LA(2);

            if ( (LA5_6=='e') ) {
                int LA5_13 = input.LA(3);

                if ( (LA5_13=='a') ) {
                    int LA5_16 = input.LA(4);

                    if ( (LA5_16=='d') ) {
                        int LA5_19 = input.LA(5);

                        if ( ((LA5_19 >= '0' && LA5_19 <= '9')||(LA5_19 >= 'A' && LA5_19 <= 'Z')||(LA5_19 >= 'a' && LA5_19 <= 'z')) ) {
                            alt5=10;
                        }
                        else {
                            alt5=6;
                        }
                    }
                    else {
                        alt5=10;
                    }
                }
                else {
                    alt5=10;
                }
            }
            else {
                alt5=10;
            }
            }
            break;
        case '0':
        case '1':
        case '2':
        case '3':
        case '4':
        case '5':
        case '6':
        case '7':
        case '8':
        case '9':
            {
            alt5=7;
            }
            break;
        case '\n':
        case '\r':
            {
            alt5=8;
            }
            break;
        case '\t':
        case ' ':
            {
            alt5=9;
            }
            break;
        case 'A':
        case 'B':
        case 'C':
        case 'D':
        case 'E':
        case 'F':
        case 'G':
        case 'H':
        case 'I':
        case 'J':
        case 'K':
        case 'L':
        case 'M':
        case 'N':
        case 'O':
        case 'P':
        case 'Q':
        case 'R':
        case 'S':
        case 'T':
        case 'U':
        case 'V':
        case 'W':
        case 'X':
        case 'Y':
        case 'Z':
        case 'a':
        case 'b':
        case 'c':
        case 'd':
        case 'e':
        case 'f':
        case 'g':
        case 'h':
        case 'j':
        case 'k':
        case 'l':
        case 'm':
        case 'n':
        case 'o':
        case 'q':
        case 's':
        case 't':
        case 'u':
        case 'v':
        case 'w':
        case 'x':
        case 'y':
        case 'z':
            {
            alt5=10;
            }
            break;
        default:
            NoViableAltException nvae =
                new NoViableAltException("", 5, 0, input);

            throw nvae;

        }

        switch (alt5) {
            case 1 :
                // Init.g:1:10: T__9
                {
                mT__9(); 


                }
                break;
            case 2 :
                // Init.g:1:15: T__10
                {
                mT__10(); 


                }
                break;
            case 3 :
                // Init.g:1:21: T__11
                {
                mT__11(); 


                }
                break;
            case 4 :
                // Init.g:1:27: T__12
                {
                mT__12(); 


                }
                break;
            case 5 :
                // Init.g:1:33: T__13
                {
                mT__13(); 


                }
                break;
            case 6 :
                // Init.g:1:39: T__14
                {
                mT__14(); 


                }
                break;
            case 7 :
                // Init.g:1:45: ENTIER
                {
                mENTIER(); 


                }
                break;
            case 8 :
                // Init.g:1:52: NEWLINE
                {
                mNEWLINE(); 


                }
                break;
            case 9 :
                // Init.g:1:60: BLANCS
                {
                mBLANCS(); 


                }
                break;
            case 10 :
                // Init.g:1:67: IDENT
                {
                mIDENT(); 


                }
                break;

        }

    }


 

}